...
bool Mesh::LoadMesh(const string& fileLocation, const string& fileName)
{
	...
	unsigned int flags = aiProcess_GenSmoothNormals | aiProcess_FlipUVs | aiProcess_JoinIdenticalVertices | aiProcess_FixInfacingNormals;
...
bool Mesh::CreateMesh(const std::vector<VertexFormat> &vertices, const std::vector<unsigned short> &indices)
{
	ClearData();
	meshEntries.resize(1);
	//materials.resize(1);

	unsigned int nrVertices = 0;
	unsigned int nrIndices = 0;
	
	meshEntries[0].materialIndex = 0; //??????????????????????????????
	meshEntries[0].nrIndices = indices.size();
	meshEntries[0].baseVertex = nrVertices;
	meshEntries[0].baseIndex = nrIndices;

	nrVertices = vertices.size();
	nrIndices = indices.size();

	this->positions.resize(nrVertices);
	this->normals.resize(nrVertices);
	this->texCoords.resize(nrVertices);
	
	this->indices = indices;
	// Reserve space in the vectors for the vertex attributes and indices
	for (unsigned short i = 0; i < nrVertices; i++)
	{
		positions[i] = vertices[i].position;
		normals[i] = vertices[i].normal;
		texCoords[i] = vertices[i].text_coord;
	}
